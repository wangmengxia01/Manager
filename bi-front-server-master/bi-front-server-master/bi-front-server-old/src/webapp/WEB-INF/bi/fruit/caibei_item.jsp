...
<div class="table-responsive">
    <table id="caibeiItem" class="table table-striped">
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=fruit_queryCaibeiItem", {
    ...
    function download() {
        $('#iframe1').attr("src", "bi?input=fruit_downloadCaibeiItem"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&fileName=" + "fruit_caibei_item.csv");
    }
