...
<div class="table-responsive">
    <table id="mjdrlianxiWin" class="table table-striped">
...
<div class="table-responsive">
    <table id="mjdrlianxiWin1" class="table table-striped">
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=mjdr_querylianxiWin", {
            timeBegin: $('#timeBegin').val(),
            timeEnd: $('#timeEnd').val(),
            offsetTimeZone: $('#offsetTimeZone').val(),
            timeInterval: "DAY"
        }, function (res) {
            if( res.indexOf( "<thead>" ) < 0 ) $('#mjdrlianxiwin').html(res);
            else {
                arr = res.trim().split('\n');
                $('#mjdrlianxiWin').html(arr[0]);
                $('#mjdrlianxiWin1').html(arr[1]);
            }
            $.unblockUI();
        });
    }
    ...
    function download() {
        $('#iframe1').attr("src", "bi?input=mjdr_downloadlianxiWin"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&fileName=" + "mjdr_lianxiWin.csv");
    }
