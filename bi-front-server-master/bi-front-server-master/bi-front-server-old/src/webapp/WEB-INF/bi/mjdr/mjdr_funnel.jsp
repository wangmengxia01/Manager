...
<div class="table-responsive">
    <table id="mjdrFunnel" class="table table-striped">
...
<div class="table-responsive">
    <table id="mjdrFunnel2" class="table table-striped">
...
<div class="table-responsive">
    <table id="mjdrFunnel3" class="table table-striped">
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=mjdr_queryFunnel", {
            timeBegin: $('#timeBegin').val(),
            timeEnd: $('#timeEnd').val(),
            offsetTimeZone: $('#offsetTimeZone').val(),
            timeInterval: "DAY"
        }, function (res) {
            if( res.indexOf( "<thead>" ) < 0 ) $('#mjdrFunnel').html(res);
            else {
                arr = res.trim().split('\n');
                $('#mjdrFunnel').html(arr[0]);
                $('#mjdrFunnel2').html(arr[1]);
                $('#mjdrFunnel3').html(arr[2]);
            }
            $.unblockUI();
        });
    }
    ...
    function download() {
        $('#iframe1').attr("src", "bi?input=mjdr_downloadFunnel"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&fileName=" + "mjdr_funnel.csv");
    }
