...
<div class="body slide">
    ...
    <div class="container-fluid left-border">
        ...
        <div class="col-md-10 col-lg-10">
            <h1><i class="fa fa-money"></i> 新增用户留存</h1>
        </div>
        ...
        <div class="col-md-5 col-lg-5">
            <h4><i class="fa fa-money"></i> 汇总表</h4>
        </div>
        ...
        <div class="col-md-5 col-lg-5">
            <h4><i class="fa fa-money"></i> 详细表</h4>
        </div>
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=user_queryInstallRetention", {
            ...
            if (res.indexOf("<thead>") < 0) $('#sumTable').html(res);
            else {
                arr = res.trim().split('\n');
                $('#sumTable').html(arr[0]);
                $('#detailTable').html(arr[1]);
            }
            $.unblockUI();
        });
    }
    ...
    function download() {
        $('#iframe1').attr("src", "bi?input=user_downloadInstallRetention"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&team=" + $('#team').val()
                + "&game=" + $('#game').val()
                + "&fileName=" + "install_retention.csv");
    }
