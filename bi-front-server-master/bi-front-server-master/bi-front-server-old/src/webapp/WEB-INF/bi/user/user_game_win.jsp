...
<div class="body slide">
    ...
    <div class="container-fluid left-border">
        ...
        <div class="col-md-10 col-lg-10">
            <h1><i class="fa fa-money"></i> 用户游戏输赢统计</h1>
        </div>
        ...
        <div class="table-responsive">
            <table id="phoneNumTable" class="table table-striped">
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=user_queryUserGameWin", {
            timeBegin: $('#timeBegin').val(),
            timeEnd: $('#timeEnd').val(),
            offsetTimeZone: $('#offsetTimeZone').val(),
            timeInterval: "DAY"
        }, function (res) {
            $('#phoneNumTable').html(res);
            $.unblockUI();
        });
    }

    function download() {
        $('#iframe1').attr("src", "bi?input=user_downloadUserGameWin"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&fileName=" + "UserGameWin.csv");
    }
</script>
