...
<div class="table-responsive">
    <table id="mtlFinance" class="table table-striped">
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=mtl2_queryJackport", {
    ...
    function download() {
        $('#iframe1').attr("src", "bi?input=mtl2_downloadJackport"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&fileName=" + "mtl2_Jackport.csv");
    }
