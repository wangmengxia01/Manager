...
<form>
    ...
    <div class="form-group col-md-2">
        ...
        <input id="limit" name="limit" type="hidden" value="20000"/>
...
<div class="col-md-5 col-lg-5">
    <h4><i class="fa fa-money"></i> 盈利玩家</h4>
</div>

<div class="row"></div>

<div class="table-responsive">
    <table id="profitRank" class="table table-striped">
...
<div class="col-md-5 col-lg-5">
    <h4><i class="fa fa-money"></i> 亏损玩家</h4>
</div>
...
<div class="table-responsive">
    <table id="deficitRank" class="table table-striped">

    </table>
</div>

<div id="usernameScript"></div>
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=texaspoker_queryRank", {
            ...
            if (res.indexOf("<thead>") < 0) $('#profitRank').html(res);
            else {
                arr = res.trim().split('\n');
                $('#profitRank').html(arr[0]);
                $('#deficitRank').html(arr[1]);
                if( arr.length > 2 ) $('#usernameScript').html(arr[2]);
            }
            $.unblockUI();
        });
    }
    ...
    function download() {
        $('#iframe1').attr("src", "bi?input=texaspoker_downloadRank"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "NONE"
                + "&limit=" + $('#limit').val()
                + "&fileName=" + "texaspoker_rank.csv");
    }
</script>
