...
<div class="body slide">
    ...
    <div class="container-fluid left-border">
        ...
        <div class="col-md-5 col-lg-5">
            <h1><i class="fa fa-money"></i> 黄钻礼包</h1>
        </div>
        ...
        <div class="col-md-5 col-lg-5">
            <h4><i class="fa fa-money"></i> 黄钻新手礼包</h4>
        </div>
        <div class="row"></div>
        <div class="table-responsive">
            <table id="YellowGiftTable" class="table table-striped">
        ...
        <div class="col-md-5 col-lg-5">
            <h4><i class="fa fa-money"></i> 黄钻每日礼包</h4>
        </div>

        <div class="row"></div>

        <div class="table-responsive">
            <table id="YellowGiftTable1" class="table table-striped">
        ...
        <div class="col-md-5 col-lg-5">
            <h4><i class="fa fa-money"></i> 黄钻等级礼包</h4>
        </div>
        <div class="row"></div>
        <div class="table-responsive">
            <table id="YellowGiftTable2" class="table table-striped">
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=yellow_queryYellow", {
            timeBegin: $('#timeBegin').val(),
            timeEnd: $('#timeEnd').val(),
            offsetTimeZone: $('#offsetTimeZone').val(),
            timeInterval: "DAY"
        }, function (res) {
            if( res.indexOf( "<thead>" ) < 0 ) $('#YellowGiftTable').html(res);
            else {
                arr = res.trim().split('\n');
                $('#YellowGiftTable').html(arr[0]);
                $('#YellowGiftTable1').html(arr[1]);
                $('#YellowGiftTable2').html(arr[2]);
            }
            $.unblockUI();
        });
    }
    ...
    function download() {
        $('#iframe1').attr("src", "bi?input=yellow_downloadYellow"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&fileName=" + "product_Yellow_gift.csv");
    }
</script>
