...
<div class="table-responsive">
    <table id="onlineTable" class="table table-striped">
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=newpoker_queryHuizong", {
            timeBegin: $('#timeBegin').val(),
            timeEnd: $('#timeEnd').val(),
            offsetTimeZone: $('#offsetTimeZone').val(),
            timeInterval: "DAY"
        }, function (res) {
            $('#onlineTable').html(res);
            $.unblockUI();
        });
    }

    function download() {
        $('#iframe1').attr("src", "bi?input=newpoker_downloadHuizong"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&fileName=" + "newpoker_huizong.csv");
    }
