...
<div class="table-responsive">
    <table id="jichuTable" class="table table-striped">
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=newpoker_queryJichu", {
    ...
    function download() {
        $('#iframe1').attr("src", "bi?input=newpoker_downloadJichu"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&fileName=" + "newpoker_downloadJichu.csv");
    }
