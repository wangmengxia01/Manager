...
<div class="col-md-5 col-lg-5">
    <h4><i class="fa fa-money"></i> 各场次抽奖次数</h4>
</div>
...
<div class="table-responsive">
    <table id="ddzchoujiang" class="table table-striped">
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=ddz_queryChoujiang", {
            timeBegin: $('#timeBegin').val(),
            timeEnd: $('#timeEnd').val(),
            offsetTimeZone: $('#offsetTimeZone').val(),
            timeInterval: "DAY"
        }, function (res) {
                $('#ddzchoujiang').html(res);
            $.unblockUI();
        });
    }

    function download() {
        $('#iframe1').attr("src", "bi?input=ddz_downloadChoujiang"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&fileName=" + "ddz_Choujiangcishu.csv");
    }
