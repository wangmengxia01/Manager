...
<div class="body slide">
    ...
    <div class="container-fluid left-border">
        ...
        <div class="col-md-5 col-lg-5">
            <h1><i class="fa fa-money"></i> 捕鱼基础报表</h1>
        </div>
...
<script>
    ...
    function query() {
        $.blockUI();
        $.post("bi?input=summary_queryFishFinance", {
            timeBegin: $('#timeBegin').val(),
            timeEnd: $('#timeEnd').val(),
            offsetTimeZone: $('#offsetTimeZone').val(),
            timeInterval: "DAY"
        }, function (res) {
            $('#financeTable').html(res);
            $.unblockUI();
        });
    }

    function download() {
        $('#iframe1').attr("src", "bi?input=summary_downloadFishFinance"
                + "&timeBegin=" + $('#timeBegin').val()
                + "&timeEnd=" + $('#timeEnd').val()
                + "&offsetTimeZone=" + $('#offsetTimeZone').val()
                + "&timeInterval=" + "DAY"
                + "&fileName=" + "summary_fish_finance.csv");
    }
</script>
